...
export default {
  ...
  methods: {
    ...
    move(event) {
       ...
       */
      let x = event.offsetX - 100; // 让鼠标在遮罩层的中间，需要减去遮罩层的宽度的一半
      if (x < 0) {
        // x的值最终是要赋值给遮罩层的left的，所以需要进行临界值的判断
        x = 0;
      }
      if (x > 200) x = 200;
      this.left = x;
      let y = event.offsetY - 100; // 让鼠标在遮罩层的中间，需要减去遮罩层的宽度的一半
      if (y < 0) {
        y = 0;
      }
      if (y > 200) y = 200;
